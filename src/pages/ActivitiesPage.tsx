...
import { CalendarCheck, Check, Clock3, Compass, EyeOff, Heart, History, LibraryBig, LoaderCircle, MapPin, Pencil, Plus, RotateCcw, Save, Search, Trash2, Upload, WalletCards, X } from 'lucide-react'
...
const ActivitiesPage = () => {
  ...
  const [catalogueFilters, setCatalogueFilters] = useState<ActivityFilters>({})
  ...
  const catalogueSource = view === 'saved' ? saved : activities
  const visibleCatalogue = useMemo(() => {
    const query = catalogueSearch.trim().toLocaleLowerCase()
    return catalogueSource.filter((activity) => {
      if (query && !`${activity.name} ${activity.description}`.toLocaleLowerCase().includes(query)) return false
      if (catalogueFilters.category && activity.category !== catalogueFilters.category) return false
      if (catalogueFilters.locationType && activity.locationType !== catalogueFilters.locationType) return false
      if (catalogueFilters.budgetLevel && activity.budgetLevel !== catalogueFilters.budgetLevel) return false
      if (catalogueFilters.energyLevel && activity.energyLevel !== catalogueFilters.energyLevel) return false
      if (catalogueFilters.durationCategory && activity.durationCategory !== catalogueFilters.durationCategory) return false
      return true
    })
  }, [catalogueFilters, catalogueSearch, catalogueSource])
  const hasCatalogueFilters = Boolean(catalogueSearch.trim() || Object.values(catalogueFilters).some(Boolean))
  ...
  return <PageTransition>
    ...
    <section className="mx-auto max-w-[1450px] px-5 pb-28 sm:px-8 lg:px-12">
      ...
      {!loading && (view === 'catalogue' || view === 'saved') && <><p className="editorial-rule">{view === 'saved' ? 'Kept close' : 'The full archive'}</p><h2 className="mt-4 font-display text-5xl">{view === 'saved' ? `${saved.length} saved ideas` : `${activities.length} visible ideas`}</h2><div className="paper-surface mt-8 rounded-[var(--radius-lg)] p-5 sm:p-6"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-7"><label className="relative block text-sm font-bold md:col-span-2 xl:col-span-2">Search ideas<Search size={17} className="pointer-events-none absolute bottom-4 left-4 text-muted" aria-hidden="true" /><input type="search" value={catalogueSearch} onChange={(event) => setCatalogueSearch(event.target.value)} placeholder="Try picnic, art, cooking…" className={`${fieldClass} pl-11`} /></label><SelectField label="Category" value={catalogueFilters.category ?? ''} onChange={(value) => setCatalogueFilters({ ...catalogueFilters, category: value ? value as ActivityFilters['category'] : undefined })}><option value="">All categories</option>{activityCategories.map((value) => <option key={value} value={value}>{pretty(value)}</option>)}</SelectField><SelectField label="Where" value={catalogueFilters.locationType ?? ''} onChange={(value) => setCatalogueFilters({ ...catalogueFilters, locationType: value ? value as ActivityFilters['locationType'] : undefined })}><option value="">Anywhere</option>{activityLocations.map((value) => <option key={value} value={value}>{activityLabels.location[value]}</option>)}</SelectField><SelectField label="Budget" value={catalogueFilters.budgetLevel ?? ''} onChange={(value) => setCatalogueFilters({ ...catalogueFilters, budgetLevel: value ? value as ActivityFilters['budgetLevel'] : undefined })}><option value="">Any budget</option>{activityBudgets.map((value) => <option key={value} value={value}>{activityLabels.budget[value]}</option>)}</SelectField><SelectField label="Energy" value={catalogueFilters.energyLevel ?? ''} onChange={(value) => setCatalogueFilters({ ...catalogueFilters, energyLevel: value ? value as ActivityFilters['energyLevel'] : undefined })}><option value="">Any energy</option>{activityEnergies.map((value) => <option key={value} value={value}>{activityLabels.energy[value]}</option>)}</SelectField><SelectField label="Duration" value={catalogueFilters.durationCategory ?? ''} onChange={(value) => setCatalogueFilters({ ...catalogueFilters, durationCategory: value ? value as ActivityFilters['durationCategory'] : undefined })}><option value="">Any length</option>{activityDurations.map((value) => <option key={value} value={value}>{activityLabels.duration[value]}</option>)}</SelectField></div><div className="mt-4 flex min-h-11 flex-wrap items-center justify-between gap-3 border-t border-line pt-4"><p role="status" aria-live="polite" className="text-sm text-muted">Showing <strong className="text-foreground">{visibleCatalogue.length}</strong> of {catalogueSource.length} ideas</p>{hasCatalogueFilters && <button type="button" onClick={() => { setCatalogueSearch(''); setCatalogueFilters({}) }} className="min-h-11 rounded-full border border-line px-4 text-xs font-bold uppercase tracking-[0.1em]"><X size={14} className="mr-2 inline" />Clear filters</button>}</div></div><div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{visibleCatalogue.map((activity) => card(activity, view === 'catalogue'))}</div>{visibleCatalogue.length === 0 && <div className="mt-8 rounded-[var(--radius-lg)] border border-dashed border-line px-5 py-16 text-center"><Search className="mx-auto text-accent" aria-hidden="true" /><h3 className="mt-5 font-display text-4xl">No ideas match yet.</h3><p className="mt-3 text-sm text-muted">Try a broader search or clear one of the filters.</p></div>}{view === 'catalogue' && hidden.length > 0 && <details className="mt-12 border-y border-line py-6"><summary className="cursor-pointer font-bold">Hidden ideas ({hidden.length})</summary><div className="mt-5 grid gap-3 sm:grid-cols-2">{hidden.map((activity) => <div key={activity.id} className="flex items-center justify-between rounded-md border border-line p-4"><span>{activity.name}</span><button type="button" onClick={() => void run(async () => { await activityService.restore(activity.id); await load() }, 'Idea restored.')} className="min-h-11 rounded-full border border-line px-4 text-xs font-bold">Restore</button></div>)}</div></details>}</>}
