  ...
  >
    <figure className="paper-surface overflow-hidden rounded-[var(--radius-md)] p-2.5">
      <div className={`image-wash relative overflow-hidden rounded-[0.35rem] bg-surface ${aspectClasses[memory.aspect]}`}>
        {memory.videoSrc ? (
          <video src={memory.videoSrc} aria-label={memory.alt} muted playsInline preload="metadata" className="h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.035]" />
        ) : memory.image ? (
          <img src={memory.image} alt={memory.alt} loading={priority ? 'eager' : 'lazy'} className="h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.035]" />
        ) : (
